<template>
  <div class="acquirer-acquisition">
    <!-- 页面标题 -->
    <div class="page-header">
      <h2 class="page-title">收购需求管理</h2>
    ...
    >
      ...
      >
        ...
        <!-- 常用地址快速填充 -->
        <el-form-item label=" " v-if="savedAddresses.length > 0">
          <div class="saved-addr-bar">
            <span class="saved-addr-label">常用地址：</span>
            <div class="saved-addr-list">
              ...
              >
                <span class="chip-name">{{ addr.contactName || '未命名' }}</span>
                <span class="chip-market">{{ addr.marketAddress }}</span>
                <span class="chip-phone">{{ addr.contactPhone }}</span>
              </div>
            </div>
          </div>
        </el-form-item>
...
const fillFromAddress = (addr) => {
  if (!addr) return
  form.value.marketAddress = addr.marketAddress || form.value.marketAddress
  form.value.contactName = addr.contactName || form.value.contactName
  form.value.contactPhone = addr.contactPhone || form.value.contactPhone
...
const loadTodayStats = async () => {
  try {
    ...
    if (res.code === 200) {
      todayCount.value = res.data?.count ?? res.data ?? 0
    }
  } catch (error) {
